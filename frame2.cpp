...
void collision_border(std::vector<particules>& particule,int nbrepart, const int WIDTH, const int HEIGHT){
    ...
    for(i = 0; i < nbrepart; i++) {
        ...
        if(particule[i].posx-1 == 0 || particule[i].posx-1 < 0){
            if(particule[i].vx < 0){
                particule[i].vx = (-particule[i].vx);
            }else if(particule[i].vy < 0)
                particule[i].vy = (-particule[i].vy);
        }else if(particule[i].posy-1 == 0 || particule[i].posy-1 < 0){
            if(particule[i].vx > 0)
                particule[i].vy = (-particule[i].vy);
            else if(particule[i].vy < 0)
                particule[i].vy = (-particule[i].vy);
        }
        if(particule[i].posx-1 == 0 && particule[i].posy-1 == 0){
            particule[i].vy = (-particule[i].vy);
            particule[i].vx = (-particule[i].vx);
        }
    }
    ...
    for(i = 0; i < nbrepart; i++) {
        for(j = 1; j < nbrepart; j++){
            ...
            if(particule[i].posx == particule[j].posx && particule[i].posy == particule[j].posy){
                if(particule[j].masse > particule[i].masse || particule[j].masse == particule[i].masse){
                    particule[i].vx = particule[j].vx;
                    particule[i].vy = particule[j].vx;
                }else{
                    particule[j].vx = particule[i].vx;
                    particule[j].vy = particule[i].vx;
                }
